        ...
        for(std::vector<std::string>::iterator c=chargeList.begin(); c!=chargeList.end();c++){
			...
			if(_genConfs->isChargeSeparated())
			{
				...
				if(*m == "d2kpi") {
					...
					if (*t == "LL") efficiencyBdt = input->getD(Form("effSig_%s_%s_%s_%s",(*t).c_str(),kstmasscut.c_str(),kshelcut.c_str(),bdtadsLL.c_str()))/input->getD(Form("effSig_%s_%s_%s_%s_25",(*t).c_str(),kstmasscut.c_str(),kshelcut.c_str(),bdtcutLL.c_str()));
					if (*t == "DD") efficiencyBdt = input->getD(Form("effSig_%s_%s_%s_%s",(*t).c_str(),kstmasscut.c_str(),kshelcut.c_str(),bdtadsDD.c_str()))/input->getD(Form("effSig_%s_%s_%s_%s_25",(*t).c_str(),kstmasscut.c_str(),kshelcut.c_str(),bdtcutDD.c_str()));
					...
					if(*c == "plus") {
						n_bu_gen[*m][*c][*t][*a] = new RooFormulaVar(Form("n_bu_gen_%s",identifier),"@0*@1*@2*@3",RooArgList(*N_kpi[*t][*a],*effVeto,*effBdt,*A[*m]));
						n_bu_fit[*m][*c][*t][*a] = new RooFormulaVar(Form("n_bu_fit_%s",identifier),"@0*@1*@2*@3",RooArgList(*N_kpi[*t][*a],*effVeto,*effBdt,*A[*m]));
					}
					else if (*c == "minus") {
						n_bu_gen[*m][*c][*t][*a] = new RooFormulaVar(Form("n_bu_gen_%s",identifier),"@0*@1*@2*@3",RooArgList(*N_kpi[*t][*a],*effVeto,*effBdt,*R[*m]));
						n_bu_fit[*m][*c][*t][*a] = new RooFormulaVar(Form("n_bu_fit_%s",identifier),"@0*@1*@2*@3",RooArgList(*N_kpi[*t][*a],*effVeto,*effBdt,*R[*m]));
					}
